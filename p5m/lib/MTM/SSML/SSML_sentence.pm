package MTM::SSML::SSML_sentence;
...
#**************************************************************#
# Create lists of features that should have SSML instructions: <phoneme>, <sub>, <break> and special.
sub createTypeLists {
...
sub ssml_insertions {
	...
	# print "ph @phoneme_types	sub @sub_types	br @break_types\n";

	# Return if SSML already set
	return $t->{ssml} if &MTM::Legacy::isDefault( $t->{ssml} ) == 0;

	# <phoneme>
	if( &MTM::Legacy::isDefault( $t->{pron} ) == 1 ) {
	...
	# <sub>
	if( &MTM::Legacy::isDefault( $t->{exp} ) == 1 ) {
...
#	}
	...
	# Return if SSML already set
	return $self if &MTM::Legacy::isDefault( $t->{ssml} ) == 0;

	# <pause>
	if( $t->{pause} =~ /\d/ ) {
		foreach my $break( @break_types ) {
...
#	my $self = shift;
#	my $chunk = shift;
#	my ( $start_tok_char_id, $char_tok_len, $start_orthography, $ssml_type, $ssml_value ) = @_;
#
