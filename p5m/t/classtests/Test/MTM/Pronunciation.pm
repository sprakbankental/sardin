...
#	is( $pron, "\'ä m \| \'ä m \| \'e t \| m \'i2: \| \'ä m" ), 'spell_all pron mm–1μm: correct.'
#}
#**************************************************#
...
sub createCompoundPronunciation : Test(4) {
	my $triplets = {
		general => [
			[
				'nasal+nasal',
				'n a $ s "a2: l - n a $ s `a2: l',
				'compound numeral pron (nasal+nasal): correct.'
			],
			[
				'nasal+nasal+nasal',
				'n a $ s "a2: l - n a $ s a2: l - n a $ s `a2: l',
		...
		initial => [
			[
				'1928+-+kvarleva',
				'n "i $ t `å n | h \'u n $ d r a | tj u2: $ g o ~ "å $ t a - k v a2: r - l `e2: $ v a',
			...
			[
				'δ+-+nasal',
				'd "e l $ t a - n a $ s `a2: l',
				'compound initial special pron (δ+-+nasal): correct.'
			],

			# TODO: change to 'tj "i2: - t v å2: - t `ä s t' when fixed.
			[
				'χ2+-+test',
				't ä s t',
			...
			[
				'LPPRT+-+nasal',
				'ä l ~ p e2: ~ p e2: ~ "ä3 r ~ t e2: - n a $ s `a2: l',
	...
	#is( $pron, "s p r \"ö l k - n a \$ s \`a2: l", 'compound initial autopron pron (sprölk+-+nasal): correct.' );

	### TODO 210823	When autopron is up working.
	#$pron = &MTM::Pronunciation::Compound::createCompoundPronunciation( 'nasal+-+sprölk' );
	#is( $pron, "n a \$ s \"a2: l - s p r \`ö l k", 'compound final autopron pron (nasal+-+sprölk): correct.' );

	### TODO 210823	When autopron is up working.
	#$pron = &MTM::Pronunciation::Compound::createCompoundPronunciation( 'nasal+-+sprölk+-+nasal' );
	#is( $pron, "n a \$ s \"a2: l - s p r ö l k - n a \$ s \`a2: l", 'compound medial autopron pron (nasal+-+sprölk+-+nasal): correct.' );
...
sub conversions : Test(2) {
	subtest "General conversions" => sub {
		...
		$pron = &MTM::Pronunciation::Conversion::convert( 'j "ä . t ex - d ,o: . l i . g a', 'JJ', 'mtm2acapela', 'jättedåliga', 'jätte+dåliga' );
		is( $pron, 'j E3 t @ d o:1 l I g a', 'convert pron (tpa2mtm): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( 'j "ä . t ex - d ,o: . l i . g a', 'JJ', 'mtm2cp_swe', 'jättedåliga', 'jätte+dåliga' );
		...
		is( $pron, "p o3 \$ r \'ö2: s", 'convert pron (cp2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( 'm u0 r ee4 ux0 s', 'JJ', 'cp2tpa', 'porös', 'porös' );
		is( $pron, "m o3 \$ r \'e2: \$ u s", 'convert pron (cp2tpa): correct.' );
		...
		is( $pron, "\"a2: v - b e3 \$ t \`a2: \$ l a", 'convert pron (cp2tpa): correct.' );
...
#		is( $pron, "r \'a: d ih u", 'convert pron (cp2mtm): correct.' );
		...
		is( $pron, 'j " e t ë d ` å: l i a', 'convert pron (tpa2tacotron_1): correct.' );
	...
	subtest "nst2tpa" => sub {
		my $pron;
		# Get rid of warning if this isn't initialised
		if( $MTM::Vars::use_dict && $MTM::Vars::use_dict eq 'NST' ) {
			plan tests => 6;
		}else {
			plan skip_all => 'No NST dict';
		}
		# nst2tpa
		$pron = &MTM::Pronunciation::Conversion::convert( '""jE$te$%do:$lI$ga', 'JJ', 'nst2tpa', 'jättedåliga', 'jätte+dåliga' );
		is( $pron, 'j "ä $ t ë $ d `å2: $ l i $ g a', 'convert pron (nst2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( '"A:l$tO$nen', 'PM', 'nst2tpa', 'Aaltonen', 'Aaltonen' );
		is( $pron, "\'a2: l \$ t å \$ n ë n", 'convert pron (nst2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( '""blaNk$%bO$s`t`ar', 'NN', 'nst2tpa', 'blankborstar', 'blank+borstar' );
		is( $pron, 'b l "a ng k $ b `å $ rs rt a r', 'convert pron (nst2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( "a\$\"lEk\$sIts\'", 'PM', 'nst2tpa', 'Aleksic', 'Aleksic' );
		is( $pron, "a \$ l \'ä k \$ s i t tj", 'convert pron (nst2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( '""bA:n`$s`t`}:$ge$%vE:$gen', 'PM', 'nst2tpa', 'Barnstugevägen', 'barn+stuge+vägen' );
		is( $pron, 'b "a2: rn $ rs rt u2: $ g ë $ v `ä2: $ g ë n', 'convert pron (nst2tpa): correct.' );

		$pron = &MTM::Pronunciation::Conversion::convert( 'del$"blA:N', 'PM', 'nst2tpa', 'Delblanc', 'Delblanc' );
		is( $pron, "d ë l \$ b l \'a2: ng", 'convert pron (nst2tpa): correct.' );
	};
}
#**************************************************#
...
sub dictionary : Test(11) {
	...
	subtest "nasal all NN" => sub {
		...
		is( $pron, "n a \$ s \'a2: l", 'dictionary main pron (nasal): correct.' );
	...
	#is( $pron, "n \'å2: \$ j a", 'dictionary multi pron (nå ja): correct.' );
	...
	subtest "zevin all" => sub {
		...
		is( $pron, "s e3 \$ v \'i2: n", 'dictionary name pron (zevin): correct.' );
	...
	subtest "neurolinguistic all" => sub {
		...
		is( $pron, "n j uë \$ r3 öw ~ l i ng \$ g w \'i \$ s t i k", 'dictionary main pron (neurolinguistic): correct.' );
	...
	subtest "AAS uc" => sub {
		...
		is( $pron, "a2: ~ a2: ~ \'ä s", 'dictionary acronym pron (AAS): correct.' );
	...
	subtest "א all" => sub {
		...
		is( $pron, "\'a \$ l ë f", 'dictionary special pron (א): correct.' );
...
sub Numeral : Test(1) {
	subtest "promounce numerals" => sub {
		...
		is( $pron, "f \'e m \| h \'u n \$ d r a \| n i \$ t i3 \$ o \~ f \"y2: \$ r \`a", 'numeral pron: correct.' );
	};
}
#**************************************************#
...
sub Stress : Test(6) {
	subtest "numerals" => sub {
		plan tests => 1;
		my $pron;
		$pron = &MTM::Pronunciation::Stress::numeralStress( "n \'i \$ t i \$ u \~ f \"y2: \$ r \`a" );
		is( $pron, "n i \$ t i \$ u \~ f \"y2: \$ r \`a", "firstPartStress word (n \'i \$ t i \$ u \~ f \"y2: \$ r \`a): correct." );
	};
	subtest "First part stress" => sub {
		plan tests => 2;
		my $pron;

		$pron = &MTM::Pronunciation::Stress::firstPartStress( 'b "i l $ d `ë r' );
		is( $pron, 'b "i l $ d ë r', 'firstPartStress word (b "i l $ d `ë r): correct.' );
	...
	subtest "Last part stress" => sub {
		...
		$pron = &MTM::Pronunciation::Stress::lastPartStress( 'b "i l $ d `ë r' );
		is( $pron, 'b `i l $ d ë r', 'firstPartStress word (b "i l $ d `ë r): correct.' );
	...
	subtest "Acronym stress" => sub {
		...
		$pron = &MTM::Pronunciation::Stress::acronymStress( "\'a2: ~ b \'e2:" );
		is( $pron, '"a2: ~ b `e2:', "acronymStress word (\'a2: ~ b \'e2:): correct." );

		$pron = &MTM::Pronunciation::Stress::acronymStress( "\'a2: ~ b \'e2: ~ s \'e2:" );
		is( $pron, "a2: ~ b e2: ~ s \'e2:", "acronymStress word (\'a2: ~ b \'e2: ~ s \'e2:): correct." );

		$pron = &MTM::Pronunciation::Stress::acronymStress( "\'a2: ~ b \'e2: ~ s \'e2: ~ d \'e2:" );
		is( $pron, "a2: ~ b e2: ~ s e2: ~ d \'e2:", "acronymStress word (\'a2: ~ b \'e2: ~ s \'e2: ~ d \'e2:): correct." );

		$pron = &MTM::Pronunciation::Stress::acronymStress( "\'a2: ~ b \'e2: ~ s \'e2: ~ d \'e2: ~ \'e2:" );
		is( $pron, 'a2: ~ b e2: ~ s e2: ~ d "e2: ~ `e2:', "acronymStress word (\'a2: ~ b \'e2: ~ s \'e2: ~ d \'e2: ~ \'e2:): correct." );
	};
	...
	subtest "English acronym stress" => sub {
		...
		$pron = &MTM::Pronunciation::Stress::englishAcronymStress( "\'ei ~ b \'i2: ~ s \'i2:" );
		is( $pron, "ei ~ b i2: ~ s \'i2:", "englishAcronymStress word (\'ei: ~ b \'i2: ~ s \'i2:): correct." );
	};
	...
	subtest "Swedish compund stress" => sub {
		...
		$pron = &MTM::Pronunciation::Stress::sv_compound_stress( "j \"ä \$ t \`ë - a \$ rt \'i \$ k ë l - n a \$ s \'a2: l" );
		is( $pron, 'j "ä $ t ë - a $ rt i $ k ë l - n a $ s `a2: l', "sv_compound_stress word (j \"ä \$ t \`ë - a \$ rt \'i \$ k ë l - n a \$ s \'a2: l): correct." );

		$pron = &MTM::Pronunciation::Stress::sv_compound_stress( "\'e n \| b \'i l d - b \"i l \$ d \`ë r" );
		is( $pron, "\'e n \| b \"i l d - b \`i l \$ d ë r", "sv_compound_stress word (\'e n \| b \'i l d - b \"i l \$ d \`ë r): correct." );
	};
}
#**************************************************#
...
sub Swedify : Test(1) {
	subtest "swedify" => sub {
		...
		# Function	swedify
		$result = &MTM::Pronunciation::Swedify::swedify( "r3 \'ei \$ d ië r3" );
		is( $result, "r \'e j \$ d e2: r", "swedify 1 (r \'e j \$ d e2: r): correct." );
	};
}
...
sub Syllabify : Test(1) {
	subtest "syllabify" => sub {
		...
		# Function	syllabify
		$result = &MTM::Pronunciation::Syllabify::syllabify( 's t "e p g n `u2: ë rn a', 'stäppgnuerna' );
		is( $result, 's t "e p $ g n `u2: $ ë $ rn a', 'syllabify 1 (s t "e p g n `u2: ë rn a, stäppgnuerna): correct.' );
